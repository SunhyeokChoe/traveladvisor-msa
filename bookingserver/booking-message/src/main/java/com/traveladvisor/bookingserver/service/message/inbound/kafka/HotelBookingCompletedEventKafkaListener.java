...
import debezium.hotel.booking_outbox.Envelope;
...
@Component
public class HotelBookingCompletedEventKafkaListener implements KafkaListMessageConsumer<Envelope> {

    private final HotelBookingCompletedEventListener hotelBookingCompletedEventListener;
    ...
    @KafkaListener(
            ...
            topics = "${kafka.topic.hotel.inbound-topic-name}")
    public void receive(@Payload List<Envelope> messages,
        ...
        messages.forEach(message -> {
            ...
            Value hotelBookingCompletedEventAvroModel = message.getAfter();
            HotelBookingCompletedEventPayload hotelBookingCompletedEventPayload =
                    kafkaMessageHelper.getEventPayload(hotelBookingCompletedEventAvroModel.getEventPayload(), HotelBookingCompletedEventPayload.class);
            ...
            try {
                switch (HotelBookingApprovalStatus.valueOf(hotelBookingCompletedEventPayload.getHotelBookingApprovalStatus())) {
                    ...
                    // 호텔 예약 승인 상태가 CANCELLED 혹은 FAILED 인 경우 예약서에 호텔 예약 취소 상태를 저장하고 예약 프로세스를 종료합니다.
                    case CANCELLED, FAILED -> {
                        hotelBookingCompletedEventListener.compensateHotelBooking(
                                bookingMessageMapper.toHotelBookingResponse(hotelBookingCompletedEventPayload, hotelBookingCompletedEventAvroModel));
                        log.info("호텔 예약이 반려 처리되었습니다. BookingId: {}", hotelBookingCompletedEventPayload.getBookingId());
                    }
                    default -> {
                        log.error("알 수 없는 호텔 예약 상태입니다. BookingId: {}", hotelBookingCompletedEventPayload.getBookingId());
                    }
                }
                ...
                if (sqlException != null && sqlException.getSQLState() != null &&
                    ...
                    log.error("[PSQLState: {}] 유니크 제약 조건 예외가 발생했습니다. 이미 처리된 예약서 ID: {}",
                            sqlException.getSQLState(), hotelBookingCompletedEventPayload.getBookingId());
                } else {
                ...
                log.error("예약서를 찾지 못했습니다. BookingId: {}", hotelBookingCompletedEventPayload.getBookingId());
            }
        });
