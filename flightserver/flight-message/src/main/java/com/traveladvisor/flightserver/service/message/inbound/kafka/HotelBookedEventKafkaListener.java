package com.traveladvisor.flightserver.service.message.inbound.kafka;
...
public class HotelBookedEventKafkaListener implements KafkaSingleMessageConsumer<Envelope> {
    ...
    @Override
    @KafkaListener(
            id = "${kafka.config.consumer.flight-consumer-group-id}",
    ...
    public void receive(@Payload Envelope message,
        ...
        Value hotelBookedEventAvroModel = message.getAfter();
        HotelBookedEventPayload hotelBookedEventPayload = kafkaMessageHelper.getEventPayload(
                hotelBookedEventAvroModel.getEventPayload(), HotelBookedEventPayload.class);
        ...
        try {
            ...
            if (sqlException != null && sqlException.getSQLState() != null &&
                ...
                log.error("[PSQLState: {}] 유니크 제약 조건 예외가 발생했습니다. 이미 처리된 예약서 ID: {}",
                        sqlException.getSQLState(), hotelBookedEventPayload.getBookingId());
            } else {
                // 그 외의 데이터베이스 예외는 밖으로 전파시킵니다. 이는 @KafkaListener가 자동으로 재시도를 수행하게 합니다.
                throw new FlightApplicationServiceException("DataAccessException 예외가 발생했습니다. 예외 메시지: "
                        + ex.getMessage(), ex);
            }
        } catch (FlightNotFoundException ex) {
            // 이 @KafkaListener는 예약서 정보가 없으면 메시지 처리를 다시 시도하지 않도록 합니다.
            log.error("예약서를 찾지 못했습니다. BookingId: {}", hotelBookedEventPayload.getBookingId());
        }
