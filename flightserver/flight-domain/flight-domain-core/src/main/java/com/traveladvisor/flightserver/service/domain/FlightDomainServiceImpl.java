...
import com.traveladvisor.flightserver.service.domain.event.FlightBookingCancelledEvent;
...
import com.traveladvisor.flightserver.service.domain.event.FlightBookingFailedEvent;
...
public class FlightDomainServiceImpl implements FlightDomainService {
    ...
    public FlightBookingEvent initializeBookingApproval(BookingId bookingId,
        ...
        if (!failureMessages.isEmpty()) {
            ...
            return new FlightBookingFailedEvent(
    ...
    @Override
    public FlightBookingEvent cancelBookingApproval(BookingId bookingId,
                                                    FlightOffer flightOffer,
                                                    List<String> failureMessages) {
        // FlightOffer가 유효한지 검증합니다.
        flightOffer.validateFlightOffer(failureMessages);
        ...
        // 검증에 성공한 경우 이를 이벤트에 기록합니다.
        log.info("항공권 예약을 정상적으로 취소했습니다. BookingID: {}", bookingId.getValue());

        flightOffer.initializeBookingApproval(bookingId, FlightBookingApprovalStatus.CANCELLED);

        return new FlightBookingCancelledEvent(
                flightOffer.getBookingApproval(),
                failureMessages,
                ZonedDateTime.now(ZoneId.of(UTC)));
    }
