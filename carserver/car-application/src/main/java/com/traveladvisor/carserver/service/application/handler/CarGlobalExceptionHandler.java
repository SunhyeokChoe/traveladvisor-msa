package com.traveladvisor.carserver.service.application.handler;
...
public class CarGlobalExceptionHandler extends GlobalExceptionHandler {
    ...
    @ResponseBody
    @ExceptionHandler(value = {CarApplicationServiceException.class})
    public ResponseEntity<ErrorResponseDto> handleException(CarApplicationServiceException ex, WebRequest webRequest) {
        log.error("[Car Domain] 예외가 발생했습니다. 내용은 다음과 같습니다.", ex);
    ...
    public ResponseEntity<ErrorResponseDto> handleException(CarNotFoundException ex, WebRequest webRequest) {
        ErrorResponseDto errorResponseDTO = new ErrorResponseDto(
                webRequest.getDescription(false),
                HttpStatus.BAD_REQUEST,
                ex.getMessage(),
