...
MSA와 DDD(Domain-Driven Design), 그리고 쿠버네티스를 학습하고 실제로 적용해보기 위해 수많은 레퍼런스를 참고해가며 만들었습니다. 이 과정에서 Saga와 Outbox 패턴의 로컬 트랜잭션 체인을 통한 데이터 일관성, 그리고 쿠버네티스가 제공하는 자동화된 스케일링(Auto Scaling), 서비스 디스커버리(Service Discovery), 선언형 구성 관리(Declarative Configuration)와 같은 높은 확장성과 운영 효율성을 미약하지만 직접 체감할 수 있었습니다. 다만 Hexagonal 아키텍처는 복잡성을 고려했을 때, 앞으로 진행할 개인 사이드 프로젝트들에는 적용을 보류할 생각입니다. 이 프로젝트를 완성하기 위해 온종일 베이스 모듈만 만들다가 Git 워킹 디렉토리를 살펴보면 80개 파일의 코드에 수정 사항이 있다고 뜨기도 합니다. 이는 Adapter, Application, Domain Core가 추상화 레이어를 통해 철저히 나뉘어 각 계층은 자기 자신만의 책임을 훌륭히 수행하고 있다는 증거이기도 하지만, 작은 규모에서는 이를 도입하기 위한 뚜렷한 이유와 목표가 없다면 코드 복잡도만 올라가고 개발 속도는 느려지는 악영향만 있을 것으로 보입니다. 그리고 잘 나누어진 계층을 통해 도메인을 보호하고 외부 요인에 영향 받지 않도록 하는 목적을 달성하기 위해 많은 노력이 들어가게 되는데, 이 과정에서 비즈니스 해결보다는 아키텍처를 위해 코드를 짜고 있다는 착각이 들기도 합니다. 다만, 팀이 잘 나누어져 있고, 팀원들이 패턴에 대한 이해도가 동등하게 있으며, 각 팀별로 도메인을 맡아 처리할 수 있는 환경이라면 도입해보는 것도 좋아 보입니다.

마지막으로 이 프로젝트를 진행한 이유 중 하나는 과거 거대 버티컬 이커머스 프로젝트를 진행하면서 쿠버네티스를 처음 도입해 봤는데, 그때 느꼈던 쿠버네티스에 대한 지식의 부족함을 보완하고, 더욱 깊이 있게 다루며 성숙한 운영 역량을 갖추고자 했습니다.
...
- Hexagonal 아키텍처를 도입해 코어 도메인을 보호하고 외부 요인과의 강결합을 없앱니다.
